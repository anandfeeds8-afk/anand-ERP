...
const approveOrderPayment = async (req, res) => {
  try {
    ...
    const message = `Advance payment approved of order #${order.orderId}`;
    ...
    const io = getIO();
    io.to(salesmanId).emit("advancePaymentApproved", {
      orderId,
      message,
      type: "advancePaymentApproved",
      senderId: accountantId,
...
const generateInvoice = async (req, res) => {
  try {
    ...
    const message = `Invoice for order #${order.orderId} has been generated by ${accountant?.name}`;
    await Notification.insertOne({
      orderId: order.orderId,
      message,
    ...
    adminIds.forEach((r_id) => {
      io.to(r_id).emit("invoiceGenerated", {
        orderId: order.orderId,
        message,
        type: "invoiceGenerated",
        senderId: accountantId,
    ...
    io.to(salesmanId).emit("invoiceGenerated", {
      orderId,
      message,
      type: "invoiceGenerated",
      senderId: accountantId,
...
const generateDueInvoice = async (req, res) => {
  try {
    ...
    const message = `Due invoice for order #${order.orderId} has been generated by ${accountant?.name}`;
    await Notification.insertOne({
      orderId: order.orderId,
      message,
    ...
    io.to(salesmanId).emit("dueInvoiceGenerated", {
      orderId,
      message,
      type: "dueInvoiceGenerated",
      senderId: accountantId,
    ...
    adminIds.forEach((r_id) => {
      io.to(r_id).emit("dueInvoiceGenerated", {
        orderId: order.orderId,
        message,
        type: "dueInvoiceGenerated",
        senderId: accountantId,
